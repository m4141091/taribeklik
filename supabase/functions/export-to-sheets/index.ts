  ...
  try {
    ...
    products?.forEach(product => {
      ...
      if (kgRows && kgRows.length > 0) {
        ...
        cellUpdates.push({ range: `Sheet1!C${rowNum}`, value: product.price_per_kg ? String(product.price_per_kg) : '' });
        cellUpdates.push({ range: `Sheet1!E${rowNum}`, value: 'פרסם' });
        cellUpdates.push({ range: `Sheet1!J${rowNum}`, value: 'וריאציה' });
      ...
      if (unitRows && unitRows.length > 0) {
        ...
        cellUpdates.push({ range: `Sheet1!C${rowNum}`, value: product.price_per_unit ? String(product.price_per_unit) : '' });
        cellUpdates.push({ range: `Sheet1!E${rowNum}`, value: 'פרסם' });
        cellUpdates.push({ range: `Sheet1!J${rowNum}`, value: 'וריאציה' });
